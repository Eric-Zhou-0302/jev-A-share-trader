...
export default function History({ lang, view, onChange, onOpen, onReady, onChanged }: { lang: Lang; view: HistoryView; onChange: (view: HistoryView) => void; onOpen: (id: string, from: string) => void; onReady: () => void; onChanged: () => void }) {
  ...
  const queryKey = JSON.stringify([query, fromDate, toDate, page, lang, revision])
  const deletionItems = records.items.map(item => ({ id: item.id, label: `${item.name} ${item.symbol} · ${item.as_of} · ${item.created_at} · ${item.id}` }))
  const manager = useRecordDeletion('analyses', lang, queryKey, loadedQuery === queryKey ? deletionItems : [], () => { setRevision(value => value + 1); onChanged() })
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(async () => {
      ...
      try {
        ...
        if (!controller.signal.aborted) { setRecords(result); if (result.page !== page) onChange({ ...view, page: result.page }) }
  ...
  }, [query, fromDate, toDate, page, lang, queryKey, manager.busy])
  ...
  return <div className="history-workspace"><div className="page-heading"><span className="eyebrow">RESEARCH ARCHIVE</span><h1>{t('history')}</h1><p>{t('historyHint')}</p></div>
    ...
    {loading || loadedQuery !== queryKey ? <div className="empty-panel" role="status"><LoaderCircle className="spin" size={28}/><p>{t('loading')}</p></div> : error ? <div className="error-message" role="alert">{error}</div> : records.total ? <section className="panel"><div className="table-scroll"><table><thead><tr><th className="selection-cell"><span className="sr-only">{t('selectRecord')}</span></th><th>{t('stock')}</th><th>{t('asOf')}</th><th>{t('decision')}</th><th>{t('horizon')}</th><th>{t('reportCreated')}</th><th>{t('manage')}</th></tr></thead><tbody>{records.items.map(item => <tr key={item.id}><td className="selection-cell"><input type="checkbox" aria-label={`${t('selectRecord')} ${item.name} ${item.id.slice(0, 8)}`} checked={manager.selected.has(item.id)} disabled={manager.busy} onChange={() => manager.toggle(item.id)}/></td><td><strong>{item.name}</strong><small>{item.symbol}</small></td><td>{item.as_of}</td><td><span className={`action-badge ${item.action ?? ''}`}>{item.action ? t(item.action) : t('noDecision')}</span></td><td>{item.horizon ? `${item.horizon} ${t('sessions')}` : '—'}</td><td>{new Date(item.created_at).toLocaleString(lang === 'zh' ? 'zh-CN' : 'en-US', { timeZone: 'Asia/Shanghai', hour12: false })}</td><td><div className="record-actions"><ReportLink id={item.id} from={source} onOpen={() => onOpen(item.id, source)}>{t('viewReport')} →</ReportLink><button className="text-button delete-button" disabled={manager.busy} aria-label={`${t('deleteRecord')} ${item.name} ${item.id.slice(0, 8)}`} onClick={() => manager.request(deletionItems.filter(record => record.id === item.id))}>{t('deleteRecord')}</button></div></td></tr>)}</tbody></table></div>
