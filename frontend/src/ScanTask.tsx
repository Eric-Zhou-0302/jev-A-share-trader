...
export default function ScanTask({ id, from, lang, view, returnLabel, onBack, onChange, onOpen, onTask, onReady }: {
  ...
  const isReset = detail?.status === 'reset' || detail?.status === 'resetting'
  ...
  return <div className="scan-task-page">
    ...
    {loading ? <div className="empty-panel" role="status"><LoaderCircle size={30} className="spin"/><p>{t('loading')}</p></div> : detail && <>
      <section className="panel scan-status"><div className="panel-heading"><div><span className="eyebrow">{detail.as_of ?? '—'}</span><h3>{t('progress')} <span className="subtle-badge">{t(detail.status as Key) ?? detail.status}</span></h3></div><div className="heading-actions">
        ...
        {isActive ? !isReset && <button className="button secondary" disabled={busy || detail.status === 'pausing'} onClick={() => control('pause')}><Pause size={14}/>{t('pause')}</button> : !isReset && <><button className="button secondary" disabled={busy || !!active || !!pollError || detail.failed === 0} onClick={() => control('retry')}><RotateCcw size={14}/>{t('retry')}</button>{(detail.remaining > 0 || detail.status === 'paused') && <button className="button primary" disabled={busy || !!active || !!pollError} onClick={() => control('resume')}><Play size={14}/>{t('resume')}</button>}</>}
        ...
        {detail.error && <p className="error-message">{detail.error[lang]}</p>}{detail.status === 'pausing' && <p>{t('pauseHint')}</p>}{detail.status === 'resetting' && <p>{t('resetHint')}</p>}{detail.status === 'reset' && <p>{t('scanResetDetail')}</p>}
      </section>
      <section className="panel"><div className="panel-heading"><h3>{t('result')}</h3><select aria-label={t('filters')} value={view.filter} onChange={event => onChange({ filter: event.target.value, page: 0 })}>{['all', 'ready', 'failed', 'skipped', 'pending'].map(value => <option value={value} key={value}>{t(value as Key)}</option>)}</select></div><div className="table-scroll"><table><thead><tr><th>{t('stock')}</th><th>{t('result')}</th><th>{t('details')}</th></tr></thead><tbody>{filtered.slice(currentPage * 100, (currentPage + 1) * 100).map(item => <tr key={item.stock.symbol}><td><strong>{item.stock.name}</strong><small>{item.stock.symbol}</small></td><td>{item.action ? <span className={`action-badge ${item.action}`}>{t(item.action)}</span> : t(item.status as Key)}{item.horizon && <small>{item.horizon} {t('sessions')}</small>}</td><td>{item.analysis_id ? <ReportLink id={item.analysis_id} from={reportSource} onOpen={() => onOpen(item.analysis_id!, reportSource)}>{t('viewReport')} →</ReportLink> : item.error?.[lang] ?? '—'}</td></tr>)}</tbody></table>{!filtered.length && <p className="scan-empty-results">{t('noScanResults')}</p>}</div><div className="pagination"><button className="button secondary" disabled={currentPage === 0} onClick={() => onChange({ ...view, page: currentPage - 1 })}>{t('previousPage')}</button><span>{currentPage + 1} / {pages} · {filtered.length}</span><button className="button secondary" disabled={currentPage + 1 >= pages} onClick={() => onChange({ ...view, page: currentPage + 1 })}>{t('nextPage')}</button></div></section>
    </>}
