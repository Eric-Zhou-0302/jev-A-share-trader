...
import { ArrowRight, BarChart3, Check, ChevronRight, CircleHelp, Clock3, Globe2, Layers3, LoaderCircle, Plus, ScanLine, Search, Settings2, Star, X } from 'lucide-react'
import { api } from './api'
import { translate } from './i18n'
import type { Analysis, Lang, Settings, Stock, Summary } from './types'
import { analysisHref, historyHref, parseRoute, reportHref, scanHref } from './routes'
...
export default function App() {
  ...
  const source = route.page === 'report' ? parseRoute(route.from) : route
  const activePage = source.page === 'report' ? 'history' : source.page
  ...
  function open(id: string, from = navigation.hash, returnScroll = window.scrollY) {
    ...
    if (parseRoute(from).page === route.page && (route.page === 'scans' || route.page === 'history')) {
  ...
  const returnLabel = t(activePage === 'scans' ? 'backToScans' : activePage === 'history' ? 'backToHistory' : 'backToAnalysis')
  return <div className="app-shell">
    <aside className="sidebar">
      ...
      <nav className="main-nav" aria-label={lang === 'zh' ? '主导航' : 'Main navigation'}>{(['workbench', 'scans', 'history'] as const).map(item => { const Icon = icons[item]; return <button aria-label={t(item)} className={activePage === item ? 'active' : ''} key={item} onClick={() => go(item === 'workbench' ? analysisHref() : `#/${item}`)}><Icon size={18}/><span>{t(item)}</span>{activePage === item && <span className="nav-dot"/>}</button> })}</nav>
    ...
    <div className="main-shell"><header className="topbar"><div className="breadcrumb"><span>WORKSPACE</span><ChevronRight size={13}/>{route.page === 'report' ? <><button onClick={() => navigate(route.from, { scroll: navigation.state.returnScroll })}>{t(activePage)}</button><ChevronRight size={13}/><strong>{loadedReport?.name ?? t('report')}</strong></> : <strong>{t(page)}</strong>}</div><div className="topbar-actions"><span className="model-status"><i className={settings?.jev_configured ? 'online' : ''}/><span>Jev</span><small>{settings?.jev_configured ? t('connected') : t('notConfigured')}</small></span><button className="language-button" onClick={() => setLang(lang === 'zh' ? 'en' : 'zh')} aria-label={lang === 'zh' ? 'Switch to English' : '切换为中文'}><Globe2 size={15}/>{lang === 'zh' ? 'EN' : '中文'}</button><button className="icon-button" aria-label={t('settings')} onClick={() => setSettingsOpen(true)} disabled={!settings}><Settings2 size={18}/></button></div></header>
      <main className="main-content">
        ...
        {route.page === 'scans' && <Scans lang={lang} configured={settings?.jev_configured ?? false} viewState={route.view} onViewChange={view => navigate(scanHref(view), { replace: true, scroll: window.scrollY })} onOpen={open} onConfigure={() => setSettingsOpen(true)} onReady={onReady}/>}
        {route.page === 'history' && <History lang={lang} view={route.view} onChange={view => navigate(historyHref(view), { replace: true, scroll: window.scrollY })} onOpen={open} onReady={onReady}/>}
