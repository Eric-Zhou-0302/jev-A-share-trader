...
export default function App() {
  ...
  return <div className="app-shell">
    <aside className="sidebar">
      ...
      <nav className="main-nav" aria-label={lang === 'zh' ? '主导航' : 'Main navigation'}>{(['workbench', 'scans', 'scanHistory', 'history'] as const).map(item => { const Icon = icons[item]; return <button aria-label={t(item)} className={activePage === item ? 'active' : ''} key={item} onClick={() => go(item === 'workbench' ? analysisHref() : item === 'scanHistory' ? scanHistoryHref() : `#/${item}`)}><Icon size={18}/><span>{t(item)}</span>{activePage === item && <span className="nav-dot"/>}</button> })}</nav>
