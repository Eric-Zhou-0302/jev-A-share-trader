## Problem and change / 问题与改动

Describe the user-visible problem and resulting behavior. / 描述用户遇到的问题和修改后的行为。

## Validation / 验证

List the checks actually run and their results; note anything not verified. / 列出实际执行的检查与结果，注明未验证部分。

## Relevant details / 相关说明

For indicators or providers, describe changes to formulas, units, adjustments, or data cutoffs. For UI changes, include screenshots when useful.

涉及指标或数据源时，说明公式、单位、复权或截止日期的变化。界面修改可附截图。
