...
- **股票列表批量扫描** — 粘贴股票代码或使用自选股，每批最多 50 只，自动去重并按顺序逐只生成结果；支持进度、暂停、恢复与失败重试。默认过滤 ST、退市整理、无成交及历史不足的股票。
...
扫描可用 `Ctrl+C` 暂停。每只股票完成行情获取后立即进入分析，无需等待整个列表。模型失败或连续 3 只数据处理失败时自动暂停；历史全市场任务保留查看，不再支持新建或恢复。更多命令、缓存行为和恢复条件见 [使用指南](docs/usage.md)。
