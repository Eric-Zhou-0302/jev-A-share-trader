...
- **市场扫描**：管理批量任务；从结果打开报告后，“返回扫描”保留原任务、范围、筛选、页码和滚动位置。
...
- 切换网页扫描范围后，进度、结果与扫描历史同步切换到对应股票池；切换本身不会启动或停止任务。
- 网页可暂停任务；CLI 使用 `Ctrl+C`。暂停会等待当前股票或准备步骤完成并保存进度，此后不再处理下一只股票。已经发出的网络请求不会被强行中断；界面在等待期间显示“暂停中”。
- 网页“重置”结束当前任务并回到待开始状态，运行中的任务会先显示“重置中”，等待当前股票或准备步骤完成。扫描历史、已完成分析和行情缓存保留；重置的任务不能继续或重试，但可从扫描历史查看结果。点击“开始扫描”会创建新任务，并按既有缓存规则复用数据。
