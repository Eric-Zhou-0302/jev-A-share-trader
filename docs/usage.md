...
- **扫描任务**：独立详情页 `/#/scans/任务ID`，集中查看进度与结果，以及暂停、继续、重试和重置。支持刷新、直接打开与浏览器前进／后退；旧版带 `job` 参数的扫描链接仍可使用。从任务打开个股报告，再逐级返回任务和扫描历史时，各层筛选、页码和滚动位置分别保留。
...
- 任务详情中的“重置”结束当前任务；运行中的任务先显示“重置中”，等待当前股票或准备步骤完成。页面随后保留此任务的状态和已完成结果。扫描历史、分析报告和行情缓存均保留，重置任务不能继续或重试。新建扫描统一从市场扫描入口发起，并按既有缓存规则复用数据。
