...
- **批量扫描**：粘贴股票代码列表或使用自选股，预览去重后的代码与数量，每批最多 50 只。支持六位代码、`SH600000`、`600000.SH`，以换行、空格或中英文逗号、分号、顿号分隔。空列表、无效格式或超限不能启动，未知股票单独记为失败。创建后进入独立任务详情。
...
侧栏依次为个股分析、批量扫描、分析记录、扫描历史。在两个记录列表中，可逐条删除，也可勾选记录后点击“删除所选”；全选只作用于本页可删除记录。确认框列出准确目标，删除最后一页的记录后自动回到有效页码。
...
扫描固定输入列表并保留输入顺序，先获取一只股票的行情，随即生成并保存分析，再处理下一只。无需等待整批行情，进度按已处理股票计算，任务页显示当前股票及失败原因。分别统计成功、跳过和失败。默认覆盖沪深京，过滤 ST／退市整理、无成交和必要历史不足的股票；默认要求至少 260 根完整日线。股票通过基础校验后进入 Jev 评估，不额外按技术信号预筛选。

- 批量扫描入口切换范围只影响待新建任务的股票池和最近任务摘要。若有正在运行的扫描，入口会显示当前任务并阻止重复启动；切换页面不会启动或停止任务。
- 网页可暂停任务；CLI 使用 `Ctrl+C`。暂停会等待当前股票或准备步骤完成并保存进度，此后不再处理下一只股票。已经发出的网络请求不会被强行中断；界面在等待期间显示“暂停中”。
- 任务详情中的“中止扫描”结束当前任务；运行中的任务先显示“中止中”，等待当前股票或准备步骤完成。页面随后保留此任务的状态和已完成结果。扫描历史、分析报告和行情缓存均保留，中止任务不能继续或重试，已完成的任务不显示中止按钮。旧版已重置记录统一显示为已中止。新建扫描统一从批量扫描入口发起，并按既有缓存规则复用数据。
...
- 模型认证或响应故障立即暂停批量分析，不再预取后续股票行情，不把失败记作“持有”。连续 3 只数据处理失败且仍有待处理股票时也会暂停，修复原因后可继续或重试。
- 网页、API 与 CLI 仅接受股票列表／自选股，每批最多 50 只。旧全市场任务保留历史与结果，但不能恢复或重试；旧入口书签会进入股票列表页面。
- API 示例：`POST /api/jobs`，请求体为 `{"scope":"batch","symbols":["000001","600000.SH"]}`；自选股使用 `{"scope":"watchlist"}`，保存创建时的自选股快照。

网页分析记录按每页 50 条展示并支持全历史筛选，完整扫描结果可在对应任务中分页查看。每批最多 50 只股票，耗时取决于列表长度、历史缓存、网络和上游限制。
...
行业参考需要数据源提供归属和同口径指数。市场宽度仅使用同日、同口径且达到覆盖要求的全市场缓存；批量股票列表不会计算或代替全市场宽度，缺失时不使用该证据。可选参考缺失会保留提示，其余可用维度仍参与分析。
